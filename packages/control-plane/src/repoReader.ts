...
import type {
  ...
  ControlPlaneSetupProgress,
  ControlPlaneSetupProgressStep,
...
function buildSetupProgress(input: {
  ...
  const externalProviders = input.config
    ? Object.entries(input.config.providers).filter(
        ([providerId, provider]) => providerId !== "playwright" && provider.enabled && provider.mode === "external"
      )
    : [];
  const workflowCriticalHigh =
    input.workflowAudit?.findings.filter((finding) => finding.severity === "critical" || finding.severity === "high").length ?? 0;
  const readinessBlocked = input.readinessReport?.summary.blocked ?? 0;
  const readinessWarnings = input.readinessReport?.summary.warnings ?? 0;
  const steps: ControlPlaneSetupProgressStep[] = [
    setupStep({
      id: "recommend",
      label: "Scan and recommend setup",
      status: input.setupRecommendation ? "complete" : "pending",
      ...
      evidence: input.setupRecommendation
        ? [
            `profile=${input.setupRecommendation.setupProfile}`,
            `target=${input.setupRecommendation.recommendedTarget.id}`,
            `selectors=${input.setupRecommendation.detectedSelectors.length}`
          ]
    ...
    setupStep({
      ...
      status: input.configError ? "blocked" : input.config ? "complete" : "pending",
      description: "Keep the repository configuration valid before planning, running, or writing workflows.",
      evidence: input.configError
        ? [`error=${input.configError}`]
        : input.config
          ? [`project=${input.config.project.name}`, `targets=${Object.keys(input.config.targets).length}`, `contracts=${input.config.contracts.length}`]
          : ["No visual-hive.config.yaml loaded."],
      command: input.config ? "visual-hive doctor" : "visual-hive recommend --write-config",
      artifacts: ["visual-hive.config.yaml"]
    }),
    setupStep({
      id: "plan",
      label: "Plan PR-safe deterministic work",
      status: input.plan ? (input.plan.items.length ? "complete" : "review") : "pending",
      ...
      evidence: input.plan ? [`mode=${input.plan.mode}`, `contracts=${input.plan.items.length}`, `targets=${input.plan.targets.length}`] : ["No .visual-hive/plan.json artifact found."],
    ...
    setupStep({
      ...
      status: input.report ? (input.report.status === "passed" ? "complete" : "blocked") : "pending",
      ...
      evidence: input.report
        ? [`status=${input.report.status}`, `passed=${input.report.summary.passed}`, `failed=${input.report.summary.failed}`]
    ...
    setupStep({
      ...
      status: input.mutationReport
        ? input.mutationReport.score >= input.mutationReport.minScore
          ? "complete"
          : "blocked"
        : "review",
      description: "Confirm contracts catch intentional UI/auth/API/layout breakage rather than only producing screenshots.",
      evidence: input.mutationReport
        ? [
            `score=${Math.round(input.mutationReport.score * 100)}%`,
            `min=${Math.round(input.mutationReport.minScore * 100)}%`,
            `survived=${input.mutationReport.results.filter((result) => result.status === "survived").length}`
          ]
    ...
    setupStep({
      ...
      status: input.triageReport ? "complete" : input.report ? "pending" : "review",
      ...
      evidence: input.triageReport
        ? [`findings=${input.triageReport.summary.findingCount}`, `classes=${Object.keys(input.triageReport.summary.classifications).join(", ") || "none"}`]
    ...
    setupStep({
      ...
      status: input.workflowAudit ? (workflowCriticalHigh ? "blocked" : "complete") : "review",
      description: "Verify PR workflows stay read-only, no-secret, artifact-uploading, and free of pull_request_target execution.",
      evidence: input.workflowAudit
        ? [`findings=${input.workflowAudit.findings.length}`, `criticalHigh=${workflowCriticalHigh}`]
    ...
    setupStep({
      ...
      status: !externalProviders.length ? "complete" : input.providerSetupPlan ? "review" : "blocked",
      ...
      evidence: externalProviders.length
        ? [
            ...
            input.providerSetupPlan ? `setupPlan=${input.providerSetupPlan.providerId}` : "setupPlan=missing"
          ]
    ...
    setupStep({
      ...
      evidence: input.readinessReport
        ? [`status=${input.readinessReport.status}`, `score=${input.readinessReport.score}`, `blocked=${readinessBlocked}`, `warnings=${readinessWarnings}`]
  ...
  const status: ControlPlaneSetupProgress["status"] = blockedSteps
    ? "attention"
...
function setupStep(step: ControlPlaneSetupProgressStep): ControlPlaneSetupProgressStep {
  return {
    ...step,
...
function setupPhase(steps: ControlPlaneSetupProgressStep[], status: ControlPlaneSetupProgress["status"]): string {
  if (status === "ready") return "ready for required PR checks";
  if (status === "not_started") return "scan repository";
  const next = steps.find((step) => step.status === "blocked" || step.status === "review" || step.status === "pending");

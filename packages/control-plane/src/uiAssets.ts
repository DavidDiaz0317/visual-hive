...
function setupDetectedWorkflows(recommendation) {
  const workflows = recommendation.detectedWorkflows || [];
  if (!workflows.length) {
    return card("Existing workflow hints", '<p class="muted">No existing GitHub workflow files were detected.</p>');
  }
  return card(
    "Existing workflow hints",
    '<p class="muted">Use these hints to decide whether Visual Hive should add new workflows or adapt existing CI. Secret values are never read or shown.</p>' +
      table(["Workflow", "Triggers", "Permissions", "Risk hints", "Visual Hive"], workflows.slice(0, 10).map(workflow => [
        '<code>' + esc(workflow.path || "") + '</code>',
        (workflow.triggers || []).join(", ") || "unknown",
        (workflow.permissions || []).join(", ") || "unspecified",
        workflowRiskHints(workflow),
        workflow.visualHiveRelated ? '<span class="ok">yes</span>' : '<span class="muted">no</span>'
      ])) +
      (workflows.length > 10 ? '<p class="muted">Showing 10 of ' + esc(workflows.length) + ' detected workflows.</p>' : "")
  );
}

function workflowRiskHints(workflow) {
  const hints = [];
  if (workflow.usesPullRequestTarget) hints.push('<span class="bad">pull_request_target</span>');
  if (workflow.usesSecrets) hints.push('<span class="warn">references secrets</span>');
  return hints.length ? hints.join("<br>") : '<span class="ok">none</span>';
}
...
function providerResultsCard(results) {
  ...
  return card("Provider results", table(["Provider", "Status", "Role", "External", "Upload", "Artifacts", "Missing env", "Message"], results.map(p => [p.label, p.status, p.deterministicRole, providerExternalPolicy(p), providerUploadSummary(p), String(p.artifactCount), p.missingEnv?.join(", ") || "none", p.message])));
}
...
function providerRunResultsCard(report) {
  ...
  return card("Provider adapter run", '<p><b>Source deterministic status:</b> ' + esc(report.deterministicStatus) + '</p><p><b>Artifacts:</b> ' + esc(report.artifactCount) + '</p>' +
    table(["Provider", "Availability", "Result", "Network", "Upload", "External calls", "Policy", "Operations", "Metadata"], report.providers.map(p => [p.label, p.availability, p.result.status, p.normalized?.networkMode || "unknown", providerUploadSummary(p.result, p.normalized), String(p.normalized?.externalCallsMade ?? 0), providerExternalPolicy(p.result, p.normalized), p.operations.map(o => o.operation + ":" + o.status).join(", "), providerMetadataSummary(p)])));
}

function providerUploadSummary(provider, normalized) {
  const upload = provider.upload || provider.result?.upload;
  if (upload) {
    return upload.status + " (" + esc(upload.stagedArtifacts ?? 0) + " staged, " + esc(upload.uploadedArtifacts ?? 0) + " uploaded)";
  }
  return normalized?.artifactSummary?.uploadMode || "n/a";
}

...
export async function runPlaywrightContracts(options: RunPlaywrightOptions): Promise<RunPlaywrightContractsResult> {
  ...
  try {
    ...
    if (error instanceof PreparedExecutionIntegrityError) {
      executionBinding = error.prepared;
      spec = error.prepared.spec;
    }
...
class PreparedExecutionIntegrityError extends Error {
  constructor(
    message: string,
    readonly prepared: PreparedExecutionBinding
  ) {
    super(message);
    this.name = "PreparedExecutionIntegrityError";
  }
}
...
async function prepareBoundExecution(options: RunPlaywrightOptions, executionPlan: Plan): Promise<PreparedExecutionBinding> {
  ...
  const prepared: PreparedExecutionBinding = {
    spec,
    nonce,
    payload,
    evidenceRoot,
  ...
  if (sha256(await readFile(spec.path)) !== generatedSpecSha256 || sha256(await readFile(spec.configPath)) !== generatedConfigSha256) {
    throw new PreparedExecutionIntegrityError(
      "Visual Hive generated Playwright spec or config does not match its execution binding.",
      prepared
    );
  }
  return prepared;

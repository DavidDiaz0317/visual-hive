...
function renderAgentRequest(issue: VisualHiveIssueCandidate, run: AgentIssueRun, rootDir?: string): string {
  ...
  return sanitizeArtifactPathsForMarkdown(rootDir ?? process.cwd(), [
    ...
    "## MCP Context Path",
    "",
    "Prefer the first-party Visual Hive MCP read-only tools when an MCP client is available. Use artifacts directly only when MCP is unavailable.",
    "",
    "- `visual_hive_get_issue_context`: selected issue, affected surfaces, linked evidence, and guardrails.",
    "- `visual_hive_read_issue_queue`: queue readiness, blocked policy, suppressions, and resolved candidates.",
    "- `visual_hive_query_visual_graph`: graph, vocabulary, unresolved references, and impact pointers.",
    "- `visual_hive_get_visual_impact`: latest issue or change blast-radius analysis.",
    "- `visual_hive_read_evidence_packet`: deterministic Evidence Packet and Visual Hive verdict summary.",
    "- `visual_hive_read_mutation_report`: mutation survivor evidence when available.",
    "- `visual_hive_get_validation_command`: exact command to run after a write-preview change.",
    "- `visual_hive_get_agent_prompt`: current bounded issue-agent prompt path and output contract.",
    "- `visual_hive_get_handoff_context`: handoff and Hive export context when the issue routes to trusted handoff.",
    "",
    "Do not use MCP output as a pass/fail oracle. It is a read-only context layer over Visual Hive artifacts; Visual Hive remains the deterministic verdict authority.",
    "",
    "## Issue",
    "",
    `- Title: ${issue.title}`,
    ...
    "- `diagnosis`: concrete failure or gap hypothesis grounded in Visual Hive evidence.",
    "- `graphNodesUsed`: Visual Graph node ids used as evidence.",
    "- `artifactsUsed`: artifact paths read.",
    "- `proposedFilesToInspect`: files, contracts, routes, selectors, or artifacts to inspect before changing anything.",
    "- `proposedChanges`: files/contracts/selectors/tests to update.",
    "- `validationCommand`: exact command to rerun.",
    "- `risks`: risks, assumptions, or cases that could make the recommendation unsafe.",
    "- `writeAccessNeeded`: boolean indicating whether a future trusted write-preview is needed.",

...
export async function recommendSetup(options: RecommendSetupOptions): Promise<SetupRecommendationReport> {
  ...
  const contract = buildContract(selector, target.id, target.config, inventory);
...
function buildContract(
  ...
  if (target.kind === "storybook") {
    return buildStorybookContract(selector, targetId, inventory);
  }
...
function buildStorybookContract(
  selector: string,
  targetId: string,
  inventory: RepoInventory
): { config: ContractConfig; reasons: string[] } {
  const story = inventory.stories[0];
  ...
  return {
    config: {
      ...
      screenshots: [
        { name: "storybook-component-desktop", route, viewport: "desktop", fullPage: true, mask: [] },
        { name: "storybook-component-mobile", route, viewport: "mobile", fullPage: true, mask: [] }
      ]
    },
    reasons: [
      story
      ...
      "Desktop and mobile Storybook screenshots give the first PR-safe component visual lane without requiring Chromatic."
    ]
  };
}
...
function configInput(input: {
  ...
  return {
    ...
    selection: {
      changedFiles: selectionRulesFor(input.target, input.contract)
    },
    mutation: {
      enabled: true,
      runOn: { schedule: true },
...
function selectionRulesFor(target: TargetConfig, contract: ContractConfig): Array<{ pattern: string; contracts: string[]; risk: "medium" | "low" }> {
  if (target.kind === "storybook") {
    return [
      {
        pattern: "src/**/*.stories.*",

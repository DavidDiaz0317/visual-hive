...
export function buildVisualRepairValidationFromArtifacts(input: BuildVisualRepairValidationArtifacts): VisualRepairValidation {
  ...
  // v2 has no digest-bound broker/executor attestation. verifyRunArtifacts
  // rejects authority/absence claims, so artifact-derived receipts remain
  // advisory until a future versioned contract can prove target binding and
  // sandbox identity without trusting producer-controlled fields.
  const authoritativeForResolution = false;
...
function verifyRunArtifacts(input: RepairRunArtifacts, phase: "before" | "after", task: VisualHiveTaskContext, session: HiveRepairSession, result: ParsedHiveRepairResult, validationAt: string): VerifiedRunArtifacts {
  ...
  if (manifest.scan.authoritativeForResolution || manifest.observations.some((observation) => observation.state === "absent")) {
    throw new Error(`Visual Hive ${phase} v2 repair bundle cannot claim resolution authority or finding absence without a versioned digest-bound broker/executor attestation.`);
  }

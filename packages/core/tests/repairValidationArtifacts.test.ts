...
describe("artifact-derived visual-hive.repair-validation.v1", () => {
  it("derives an advisory blocked receipt while preserving actual reports, contexts, and images", async () => {
  ...
  it.each([
    ...
    ["finding absence", { afterFinding: "absent", afterAuthoritativeForResolution: true }]
  ] as const)("rejects a crafted v2 bundle claiming %s without broker/executor attestation", async (_claim, options) => {
    const fixture = await repairFixture(options);
    expect(() => buildVisualRepairValidationFromArtifacts(fixture.input)).toThrow(
      "cannot claim resolution authority or finding absence without a versioned digest-bound broker/executor attestation"
    );
  });
  ...
  it("blocks missing evidence and keeps a still-present advisory finding open without trusting model claims", async () => {
    ...
    expect(failedReceipt.verdict).toBe("blocked");
  ...
  it("blocks missing baselines and incomplete regression inventory, and fails a new deterministic regression", async () => {
    ...
    expect(failedRegression.verdict).toBe("blocked");
  });
...
async function repairFixture(options: FixtureOptions = {}): Promise<FixtureResult> {
  ...
  const after = await makeRun({
    ...
    finding: options.afterFinding === "missing" ? undefined : (options.afterFinding ?? "present"),
...
async function makeRun(input: MakeRunInput): Promise<BuildVisualRepairValidationArtifacts["before"]> {
  ...
  const bundle = await writeVisualHiveBundle({
    ...
    scan: { scope: "full", authoritativeForResolution: input.authoritativeForResolution ?? false, evaluatedContracts: ["contract.card", "contract.secondary"], evaluatedFiles: ["src/App.tsx"], testPlanVersion: "plan-v1", toolRegistryVersion: "tools-v1" },
    observations,
    producerVersion: "0.3.2",
    producerGitCommit: commit("c"),
    bundleId: input.phase === "before" ? "bundle.before" : "bundle.after",
    ...(input.authoritativeForResolution ? {} : { purpose: "repair-validation" as const }),

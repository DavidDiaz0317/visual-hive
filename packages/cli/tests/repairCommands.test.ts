...
describe("trusted repair CLI operations", () => {
  ...
  it("rejects changed source bytes and corrupted durable task evidence", async () => {
    ...
    const storedAsset = path.join(fixture.storeRoot, ...visualRepairSessionRelativeRoot({ taskId: fixture.task.taskId, repository: fixture.task.repository.name, taskContextDigest: fixture.task.contextDigest }).split("/"), "task-assets", "assets", "reference.png");
  ...
  it("publishes one producer-faithful immutable advisory receipt under concurrent valid validation", async () => {
    const fixture = await validationFixture();
    ...
    expect(concurrent[0]).toMatchObject({ verdict: "blocked", closureRecommendation: "keep_open" });

...
`visual-hive issues --write` converts deterministic failures, mutation survivors, coverage gaps, workflow safety findings, readiness blockers, and handoff work items into stable deduplicated issue candidates. Each candidate includes a `dedupeFingerprint`, severity, issue kind, owning agent hint, linked Evidence Packet/repo map/Hive artifacts when available, reproduction and validation commands, affected surface metadata, and guardrails. When the repository map proves an exact source path for a failed contract or discovery gap, affected surface metadata also carries `sourceFile`; this is evidence for Hive's existing repair-path policy, not permission to edit that file. New candidates also carry signed-publication metadata: `publicationRole` (`canonical`, `derivative`, or `aggregate`), a stable URI-safe `rootCauseKey`, and sorted `blockedByRootKeys` for aggregates. Mutation roots use the mutation operator, target, and sorted contract set (for example `mutation/api-500/localPreview/dashboard-shell`); repository test-adequacy roots use the testing layer (for example `test-adequacy/repository/testing-layer:2`). Mutation maintenance/coverage copies and the unit-map copy are derivatives, while a readiness summary becomes an aggregate only when every blocked gate can be linked from structured evidence. Unprovable linkage fails open as an independent canonical candidate instead of hiding work. Existing standalone issue dedupe and publication behavior remains unchanged. The command also writes `.visual-hive/issues.md`, `.visual-hive/issue-queue.json`, and `.visual-hive/setup-issue.md`.

The verified issue and Hive-bundle contracts accept only Visual Hive's closed set of deterministically produced issue kinds and exact producer kind/`owningAgentHint` pairs. `issueKind` identifies the finding Visual Hive proved; the separately digest-bound `owningAgentHint` is routing guidance that lets Hive choose among its existing specialists after independently verifying provenance. A hint never grants lifecycle, baseline, GitHub, merge, or verdict authority. Unknown kinds, generic CI/build/security/architecture/discovery claims without a deterministic producer, arbitrary owner strings, and mismatched kind/hint pairs fail closed. Every lifecycle observation includes at least one relative evidence artifact, its source packet, affected contracts when available, and a bounded validation command; Visual Hive retains final deterministic verdict authority.

For `visual-hive.bundle.v3`, relative evidence names are not proof by themselves. Every observation's canonical `sourceArtifact` and every path in `sourceArtifacts` must be present in the caller's explicit bounded artifact request, listed as a regular file in the complete content-addressed artifact index, copied into the compact bundle, and bound by the same indexed byte count and SHA-256 digest. Missing, merely indexed-but-unrequested, duplicated, or stale/tampered observation evidence fails publication; the writer never silently adds claimed evidence outside the requested file and byte limits.

`visual-hive hive bundle` copies the same publication metadata into each lifecycle observation and binds it into the bundle digest. Proven repository paths are normalized into the sorted unique `affectedFiles` array so Hive can enforce its installed allowlist before repair dispatch; missing path evidence remains an empty array and never grants scope. Canonical observation repository identity is derived from `repository + rootCauseKey`; derivative and aggregate observations retain their candidate fingerprint identity. Metadata arrays are normalized to sorted unique values. New v3 writers set `digestAlgorithm: visual-hive.bundle.content-addressed-digest.v1`; new v2 writers set `visual-hive.bundle.publication-digest.v1`, including authoritative bundles with no observations. Legacy `visual-hive.bundle.v2` manifests may omit `digestAlgorithm` only when every observation omits all three publication fields; those manifests retain their byte-exact original digest format. A marked manifest requires all three fields on every observation. Unknown algorithms, partial metadata, mixed legacy/publication observations, marked legacy observations, and unmarked publication observations are rejected.
...
Path: `.visual-hive/adapters/lifecycle-plan.json`

Schema: `schemas/visual-hive.adapter-lifecycle-plan.schema.json`

`visual-hive adapters manage` turns Tool Registry lifecycle metadata into deterministic repository-specific `install`, `update`, `use`, `skip`, or `replace` decisions for the supported open-source adapters. Signals include configured screenshot-contract count, npm/package-lock state, exact ODiff version and registry integrity, runtime support, pull-request lane safety, and VRT credential names present or missing. Credential values are never written.

Plan mode makes no dependency or external-network changes. Explicit `--apply` may install/update only the exact managed ODiff package and records executable-health plus identical/different golden-parity results in the action list. The artifact always records Playwright as primary evidence, Visual Hive as verdict authority, and `externalCallsMade: 0`; VRT provisioning/upload remains a separate trusted action.
...
The artifact index inventories files under `.visual-hive`, classifies renderable artifacts, and stores sanitized previews for text-like files. Every entry records its byte size and SHA-256 digest. Image files are linked for rendering through the Control Plane image endpoint, while JSON, Markdown, logs, YAML, text, and generated specs receive redacted previews. `visual-hive artifacts --repo <path>` can index setup artifacts before a `visual-hive.config.yaml` exists, which keeps `.visual-hive/repo-map.json`, `.visual-hive/repo-context.md`, `.visual-hive/recommendations.json`, and `.visual-hive/setup-pr-plan.json` catalog-backed during first-time onboarding.

`visual-hive artifacts --complete` is the transport-integrity form used by the final pipeline and hosted Hive bundle flow. It scans the complete bounded `.visual-hive` tree after final report bytes are sealed, excludes only the recursive bundle payload, and records discovered, indexed, and omitted counts. A complete index is valid only when every eligible artifact is represented and `omittedArtifactCount` is zero. The seal is replaced atomically; if final traversal fails, any prior complete seal is replaced by an explicit `complete: false` failure receipt. Hosted bundle creation fails closed for a missing, partial, stale, non-content-addressed, or non-exact index; Hive independently verifies the exact normalized path, size, and digest set before accepting the source artifact.

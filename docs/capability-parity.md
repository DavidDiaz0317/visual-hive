...
The reviewed `run` and `pipeline` command contracts include the optional `--runtime-sidecar <path>` evidence output. The path must name a new `runtime.json` below the config root's dedicated `.visual-hive/proof/` subtree and delegates to the existing execution-bound Playwright runtime identity; protected source, report, bundle, approval, and baseline paths are outside that lane. Omitting it preserves the prior execution and artifact behavior.

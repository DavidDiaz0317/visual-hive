...
## Durable repair namespace

Repair state is isolated under `.visual-hive/repair/sessions/<content-addressed-session-id>/`. The session identifier is derived from the task ID, full repository identity, and task-context digest; those full identities remain inside `task-context.json` and are revalidated on every reuse. New sessions contain caller-declared task paths only below the dedicated `task-assets/` namespace, plus immutable `runs/run.<request-id>/` capture records and content-addressed bundle, bounded interrupted-attempt archives, and `validations/<validation-id>.json` receipts. Random `runs/exec.<nonce>/` directories are execution-only staging areas and are removed after a completed capture; if execution is interrupted they remain covered by the same bounded, fail-closed session inventory. The reader retains verified compatibility with legacy sessions that stored task assets at the session root, but new producers cannot write that broad layout.

These fixed and dynamic paths are part of the frozen capability inventory. A release cannot silently omit, rename, or weaken one: the capability-parity gate hashes their producer contracts, and Hive handoff bundles carry the complete content-addressed parity/index evidence. Task asset paths are caller-declared but bounded and contained; the catch-all session artifact surface records that open path set without granting lifecycle authority to it.
...
The current v2 artifact bundle has no versioned, digest-bound broker/executor receipt. Artifact-based repair validation therefore rejects any v2 bundle that claims `scan.authoritativeForResolution: true` or publishes an `absent` finding, regardless of `source.trusted`, source event, URL, command target, or caller-supplied purpose. Its deterministic result remains `verdict: blocked` and `closureRecommendation: keep_open`. Resolution authority can be added only through a future versioned contract that binds sandbox identity, target-binding kind, repository checkout or deployment SHA, and the broker receipt into the verified digest; none of those facts are inferred from unsigned or non-digest-bound fields.

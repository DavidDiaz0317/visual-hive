...
- Schema/catalog drift verification through `visual-hive schemas verify`, which checks tracked JSON Schema `$id` values and evidence-resource enums against the shared core evidence-resource catalog so MCP resources, Tool Registry cards, Agent Packet allowed tools, Context Ledger tool-call metadata, artifact-index metadata, and Control Plane links do not silently diverge. The root `npm run schema:verify` path writes `.visual-hive/schema-catalog.json`, which is now catalog-backed as `visual-hive://schema-catalog` / `visual_hive_read_schema_catalog` so reviewers and agents can inspect schema/catalog health without re-running maintenance checks. The generated `.visual-hive/mcp-manifest.json` is also catalog-backed as `visual-hive://mcp-manifest` / `visual_hive_read_mcp_manifest`, including setup-only manifests generated before a config exists. Repo-intelligence outputs are now part of that same setup-safe resource surface: `.visual-hive/repo-map.json` is `visual-hive://repo-map` / `visual_hive_read_repo_map`, carries its own `outputResource` row when newly generated, and `.visual-hive/repo-context.md` is `visual-hive://repo-context` / `visual_hive_read_repo_context`.
...
- MCP and Tool Registry provider evidence alignment: `visual_hive_read_provider_decisions`, `visual_hive_read_provider_setup_plan`, `visual_hive_read_provider_handoff`, `visual_hive_read_provider_results`, and `visual_hive_read_provider_upload_manifest` are read-only local evidence tools, their resources are exposed through `visual-hive mcp`, and provider-specialist/handoff/review agents may inspect provider decisions, setup plans, handoff manifests, provider results, or Argos upload manifests without gaining credential, upload, external-network, provider-gating, or verdict authority.
- Provider decision evidence should remain catalog-backed: `.visual-hive/provider-decisions.json` is `visual-hive://provider-decisions` / `visual_hive_read_provider_decisions`, and newly generated logs carry matching `outputResource` rows. Reading this artifact does not enable credentials, billing, provider uploads, provider API calls, provider gating, or verdict overrides.
- Provider setup and handoff evidence should remain catalog-backed: `.visual-hive/provider-setup-plan.json` is `visual-hive://provider-setup-plan` / `visual_hive_read_provider_setup_plan`, `.visual-hive/provider-handoff.json` is `visual-hive://provider-handoff` / `visual_hive_read_provider_handoff`, and newly generated reports carry matching `outputResource` rows. Reading these artifacts does not enable credentials, billing, provider uploads, provider API calls, provider gating, or verdict overrides.
- Provider-results evidence should remain catalog-backed: `.visual-hive/provider-results.json` is read-only local provider evidence under `visual-hive://provider-results` / `visual_hive_read_provider_results`, and newly generated provider mock/upload result reports now carry an `outputResource` row with that same identity. It may show provider readiness, mock adapter operations, dry-run/upload status, staged artifact counts, sanitized provider output, and missing credential names, but reading it does not authorize credentials, external uploads, provider API calls, provider gating, or verdict overrides.
- Readiness gate evidence is also catalog-backed as `visual-hive://readiness-gate` / `visual_hive_read_readiness_gate`, and newly generated readiness reports carry an `outputResource` row with that same identity, so agents and trusted workflow consumers can inspect go/no-go readiness without scraping raw reports, rerunning targets, or treating guidance as a verdict override.
...
Testing-layer evidence now follows the same catalog-backed path: `.visual-hive/testing-layers.json` is read-only local evidence under `visual-hive://testing-layers` / `visual_hive_read_testing_layers`, and newly generated layer reports carry an `outputResource` row with that same identity. Preserve matching Tool Registry, MCP, Agent Packet metadata, Context Ledger metadata, artifact-index metadata, Control Plane links, docs, schemas, and tests. The Markdown summary remains a normal human-readable artifact. Preserve existing CLI behavior; this is drift prevention and agent-forward correctness, not a new verdict source.
...
Coverage evidence should also remain catalog-backed: `.visual-hive/coverage.json` is read-only local evidence under `visual-hive://coverage-map`, and newly generated coverage reports now carry an `outputResource` row with that same identity. `.visual-hive/coverage-recommendations.json` is read-only local evidence under `visual-hive://coverage-recommendations` / `visual_hive_read_coverage_recommendations`. It may contain guarded config snippets and apply IDs, but reading either artifact does not authorize applying config edits, editing tests, running targets, or changing the Visual Hive verdict. Preserve explicit `--apply` diff preview and `--yes` write confirmation as the only local write path.
...
Workflow safety evidence should remain catalog-backed: `.visual-hive/workflows.json` is read-only local evidence under `visual-hive://workflow-audit` / `visual_hive_read_workflow_audit`, and newly generated workflow audit reports carry an `outputResource` row with that same identity. It can show PR permission posture, secret usage, `pull_request_target` findings, artifact upload posture, and trusted `workflow_run` issue/handoff safety, but reading it does not authorize writing workflows, granting secrets, creating issues, or executing untrusted PR code.

Baseline governance evidence should remain catalog-backed: `.visual-hive/baselines.json` is read-only local evidence under `visual-hive://baseline-review` / `visual_hive_read_baseline_review`, `.visual-hive/baseline-approvals.json` is read-only local evidence under `visual-hive://baseline-approvals` / `visual_hive_read_baseline_approvals`, and `.visual-hive/baseline-rejections.json` is read-only local evidence under `visual-hive://baseline-rejections` / `visual_hive_read_baseline_rejections`. Newly generated baseline review queues and approval/rejection logs now carry `outputResource` metadata with those same identities. These artifacts may show created, missing, failed, approved, rejected, and pending screenshot review decisions, but reading them does not authorize approving, rejecting, copying, or updating baselines. Preserve explicit human confirmation and trusted write boundaries for baseline changes.

Deterministic report evidence should remain catalog-backed: `.visual-hive/report.json` is read-only local evidence under `visual-hive://latest-report` / `visual_hive_read_latest_report`, and newly generated deterministic reports now carry an `outputResource` row with that same identity. It may show selected targets/contracts, lifecycle events, selector/screenshot/console/page/network evidence, provider normalization, and verdict contributions, but reading it does not authorize rerunning targets, approving baselines, posting GitHub comments, calling providers, calling Hive, or overriding the Visual Hive verdict.

Active plan evidence should remain catalog-backed: `.visual-hive/plan.json` is read-only local evidence under `visual-hive://latest-plan`, and newly generated default plans now carry an `outputResource` row with that same identity. Sidecar plans such as `.visual-hive/plan.canary.json`, `.visual-hive/plan.full.json`, or custom `--output` files should not claim the `latest-plan` identity; `.visual-hive/plans.json` remains the catalog-backed lane-comparison artifact for those sidecars. Reading plans does not run targets, change selection policy, create issues, call providers, call Hive, or override the Visual Hive verdict.
...
Generated demo artifact consistency is now expected baseline for the catalog-backed evidence-resource model. Context Ledger multi-resource evidence exposure is also expected baseline: Context Ledger tool calls may carry `evidenceResources[]` in addition to compatibility `evidenceResourceId` / `evidenceResourceUri` fields when one command produced multiple catalog-backed artifacts, and the Control Plane snapshot/expert evidence should preserve those links. `demo:evidence-resources` verifies that `.visual-hive/context-ledger.json`, `.visual-hive/control-plane-snapshot.json`, `.visual-hive/artifacts-index.json`, `.visual-hive/mcp-manifest.json`, and role-specific Agent Packets expose the same catalog-backed resource IDs, URIs, artifact paths, descriptions, and read-tool names for triage, issue, PR comment, prompt, repair, missing-test, provider, repo-intelligence, and support evidence when those artifacts exist. It now also checks embedded `outputResource` rows for latest plans, latest deterministic reports, coverage-map, mutation-report, provider-results, baseline review, baseline approvals, and baseline rejections when those files are generated. The checker also supports `--root <repo> --profile general`, which is used by KubeStellar planning artifacts, consumer install smoke output, and `smoke:console` real-console dogfood output to verify catalog-backed evidence identity for non-demo generated artifacts. The console dogfood smoke now covers external-repo repo-intelligence and setup onboarding artifacts at the console repo root (`.visual-hive/repo-map.json`, `.visual-hive/repo-context.md`, `.visual-hive/recommendations.json`, and `.visual-hive/setup-pr-plan.json`) and config-scoped evidence artifacts under `web/e2e/.visual-hive`; it verifies repo-map `outputResource` metadata, setup output-resource metadata, zero setup external calls, safe generated workflow posture, setup-only MCP resource identity, and current Agent Packet budget policy through `budgets.allowExternalNetwork=false` and `budgets.maxExternalCostUsd=0`, so role packets remain no-network/no-cost unless a future trusted mode explicitly changes policy. `.visual-hive/plans.json` remains catalog-backed as `visual-hive://plan-lanes` / `visual_hive_read_plan_lanes`, and newly generated lane summaries now carry their own `outputResource` row so sidecar PR/canary/full/schedule/docs-only plans are reachable as read-only lane evidence without making each sidecar plan a separate hard-coded resource. `.visual-hive/history.json` remains catalog-backed as `visual-hive://run-history` / `visual_hive_read_run_history`, and newly generated run histories now carry their own `outputResource` row so flake, baseline stability, mutation adequacy, runtime, and cost trend evidence can be handed to agents without path guessing or verdict override. Future agents should preserve that check and update it when new catalog-backed generated artifacts become part of demo, consumer, KubeStellar, or dogfood acceptance.

Immediate next step for a resumed goal run: preserve the same catalog-backed consistency discipline while moving to the next small vertical slice. The real-console dogfood path now includes repo-intelligence setup artifacts, setup recommendations, setup PR plans, MCP manifest resources, config-scoped Evidence Packet, Context Ledger, Agent Packets, artifact index, and Control Plane snapshot with consistent resource IDs, URIs, artifact paths, descriptions, and read-tool names. Future work should build on that by tightening external-repo setup guidance, trusted Hive handoff consumption, Control Plane clarity, or budget-governed agent execution without leaking secrets, calling providers, calling LLMs, or granting agents verdict authority.

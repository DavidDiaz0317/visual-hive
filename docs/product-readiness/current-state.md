...
- The GitHub App package now includes a guarded installation-token issue client. It remains blocked unless `VISUAL_HIVE_GITHUB_APP_LIVE=true`, `VISUAL_HIVE_GITHUB_APP_LIVE_ISSUE_WRITE=true`, and GitHub App id/private-key/installation/webhook-secret env vars are configured. Tests mock the live GitHub API path and prove create/update-by-dedupe behavior without real network calls.

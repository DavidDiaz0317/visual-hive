...
| --- | --- | --- | --- | --- |
| Product branch | `git rev-parse HEAD` | Canonical branch is `main` | `86cc790d58786ee2a867fec89e6066af46a59654` before local hardening commit | Pass |
...
| Tests | `npm test` | Unit/integration tests pass | 8 files, 362 tests passed | Pass |
...
| Product path leak scan | Issue-facing generated artifacts under `examples/demo-react-app/.visual-hive` | No local absolute paths in issue-facing artifacts | Passed | Pass |
...
| Area | Command / Proof | Expected | Actual | Status |
| --- | --- | --- | --- | --- |
| Demo-site branch | `git rev-parse HEAD` in `visual-hive-demo-site` | Canonical client branch is `main` | `7662561e2106fe4ee944a5fcfce403dd43894bf9` before local hardening commit | Pass |
...
| GitHub Actions remote proof | Product/demo workflows | Workflows are committed and triggerable | Local workflow audit passes; remote workflow run not triggered in this validation pass | Pending after push |

...
| Tests | `npm test` | Unit/integration tests pass | 370 tests passed, including agent artifact validation coverage | Pass |
...
| Public site typecheck | `npm run site:typecheck` | Static Vite info site TypeScript checks pass | Passed | Pass |
| Public site build | `npm run site:build` | Vercel-targeted static site builds to `apps/visual-hive-site/dist` | Passed; Vite output written to ignored `apps/visual-hive-site/dist` | Pass |
| Public site smoke | Bounded local Vite preview on `127.0.0.1:4329` with Playwright desktop/mobile checks | Hero, CTAs, issue loop, safety section, and link targets render without horizontal overflow | Passed; screenshots written to ignored `.visual-hive/site-smoke-*.png` | Pass |
...
| Public site deploy | Vercel project linked with root `apps/visual-hive-site`; `npx vercel --prod` | Optional live deployment only when Vercel auth/project is available | Not run in local validation unless credentials/project link are present | Blocked by missing Vercel project link |

...
| Product CI | GitHub Actions run `28910330503` | Product CI passes on `main` | Passed for commit `df8e119f` | Pass |
| Product Proof | GitHub Actions run `28910330506` | Product proof passes on `main` | Passed for commit `df8e119f`, including `github-app:smoke:server`, `github-app:smoke:artifacts`, `github-app:smoke:live`, pinned workflow actions, MCP smoke, and issue-agent proof | Pass |
...
| Codex CLI no-write agent | `codex --help`; `node packages/cli/dist/index.js agent issue-runner --config examples/demo-react-app/visual-hive.config.yaml --issue-index 0 --codex-command codex --codex-discovery-timeout-ms 5000 --format json` | CLI help should be callable before live Codex adapter execution; if the local Codex binary cannot execute, Visual Hive should record a blocked no-write artifact with zero unsafe counters and actionable remediation | `codex.exe` failed with `Access is denied` / `spawn EPERM`; Visual Hive recorded `status: blocked`, a specific blocked reason, zero unsafe counters, and recommendations to pass a runnable `--codex-command` or use a repo-local deterministic agent | Blocked by local Codex CLI execution policy; product behavior passes |

...
The harness creates local Git bundles, clones them into a disposable container, and freezes pristine root-owned reference clones before any target-controlled install or build runs. Root verification never reads the target-owned Git config or index: inherited, system, and global Git configuration plus execution-capable fsmonitor, hook, replacement-object, and optional-lock behavior are disabled, while tracked bytes and executable modes are compared directly against objects in the read-only-bundle references. The references also provide the ignore rules and index used to detect unexpected untracked files. The Visual Hive build and root-owned attestations are then made unavailable for target writes; the target clone, target commands, and CLI execution all run as the image's unprivileged `pwuser`. After that user is quiescent, only the exact verifier evidence allowlist enters a validated read-only handoff state so the capability-dropped root verifier can read it: the runtime sidecar in both modes, the artifact index in preliminary mode, and the four statically named candidate/diff PNGs in candidate mode. No source, baseline, raw result, or directory-wide permission is relaxed. The container uses private IPC with a fixed 2 GiB shared-memory allocation, drops every Linux capability, enables `no-new-privileges`, publishes no ports, and receives only the read-only bundle mount. It binds the exact built CLI, config loader, package dist digest, Playwright package, Chromium executable, OS, Node runtime, reviewed config, and non-root execution identity, and rechecks those identities after execution. It runs the target source preflight exactly once, disconnects every Docker network, proves both Docker control-plane isolation and failed active egress, and only then invokes:

...
The CLI and Control Plane use the same core governance helper to record provider decisions in `.visual-hive/provider-decisions.json`. These decisions are local audit evidence only: skip a provider for now, review it later, or approve it for a future trusted setup review. New decision logs are catalog-backed as `visual-hive://provider-decisions` / `visual_hive_read_provider_decisions` so MCP clients, Agent Packets, the artifact index, and the Control Plane can inspect the same governance evidence without guessing paths. Recording or reading a decision does not create credentials, enable billing, upload screenshots, call a provider API, enable provider gating, or change the Visual Hive verdict. `visual-hive risk` and `visual-hive readiness` load the decision log so governance choices are visible before a team enables trusted provider-backed lanes.

...
- Hive-native export display is artifact-only. The Control Plane reads `.visual-hive/hive/hive-export.json`, `beads.json`, `knowledge-facts.json`, `knowledge-graph.json`, `issue-context.md`, `repair-work-orders.json`, `hive-agent-policy.json`, and wiki-vault pages when present, but it does not call Hive, create Beads, or execute repair work. Repair rows always show the Visual Hive-owned final validation command and forbidden actions so the UI does not imply autonomous pass/fail authority.
- Hive export mode preview actions execute only allowlisted `visual-hive hive export --dry-run --mode ...` commands for `advisory`, `measured`, and `repair_request`. `guarded_repair` and `full` are intentionally displayed as governed future/trusted states and are not launched from the local UI.

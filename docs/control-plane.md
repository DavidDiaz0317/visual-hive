...
- Hive export mode policy from `.visual-hive/hive/mode-comparison.json`, including side-by-side no-network previews for `advisory`, `measured`, `repair_request`, `guarded_repair`, and `full` modes, a recommended mode, emitted artifact counts, comparison artifact links, and fixed preview commands. Beginner mode highlights the first three local preview choices; expert mode exposes guarded and full blocked policy states. The UI explains what each mode emits, keeps `guarded_repair` and `full` as policy states rather than casual local actions, and never calls Hive from the browser.

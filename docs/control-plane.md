...
- Hive-native export display is artifact-only. The Control Plane reads `.visual-hive/hive/hive-export.json`, `beads.json`, `knowledge-facts.json`, `knowledge-graph.json`, `issue-context.md`, `repair-work-orders.json`, and `hive-agent-policy.json` when present, but it does not call Hive, create Beads, or execute repair work.

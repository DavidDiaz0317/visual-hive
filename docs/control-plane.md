...
- Hive export mode policy from `.visual-hive/hive/mode-comparison.json`, including side-by-side no-network previews for `advisory`, `measured`, and `repair_request` modes, a recommended mode, emitted artifact counts, comparison artifact links, and fixed preview commands. The UI explains what each mode emits, keeps `guarded_repair` and `full` as policy states rather than casual local actions, and never calls Hive from the browser.

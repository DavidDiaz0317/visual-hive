...
Write the same snapshot as an offline artifact for agents, CI summaries, or schema validation:

```bash
visual-hive snapshot --repo . --config visual-hive.config.yaml
```
...
- Run Profiles for curated local workflows such as PR acceptance, canary health planning, full PR-safe planning, triage refresh, mutation adequacy audit, coverage improvement generation, provider governance review, and portfolio refresh. Profiles are built from server-generated runbook command IDs, not browser-provided shell text. Each profile exposes `enabled`, `runnable`, `blockedReason`, and `blockedReasons` in the snapshot so the UI, agents, and CLI can distinguish runnable local profiles from missing-command, guidance-only, trusted-only, or secret-bearing profiles without guessing from raw command strings.
...
- Guidance state from the loaded config, plan, report, mutation, workflow, Evidence Packet, Handoff Packet, and Agent Packet artifacts. The Start / Quality Cockpit view uses this to show a canonical setup/adoption checklist from first config through PR-safe planning, deterministic evidence, visual review, mutation adequacy, agent handoff packaging, and safe workflow enablement. The same `guidanceState.adoptionChecklist` appears in `/api/snapshot` so agents and future clients receive the same beginner-friendly next-step model as the React UI. Each checklist item includes the linked runbook command ID, command label, command text, safety class, expected artifacts, whether it can run from the current Control Plane mode, and the blocked reason when it is read-only, trusted-only, secret-bearing, or guidance-only.
- Snapshot contract validation through `schemas/visual-hive.control-plane-snapshot.schema.json`, which documents the local `/api/snapshot` surface used by the React UI, smoke tests, and future MCP/agent clients. Nested artifacts such as reports, Evidence Packets, Hive exports, and provider results keep their own schemas; the snapshot schema locks the shared guidance, runbook, profile, navigation, failure, artifact, and adoption-checklist surface. Artifact entries use the shared evidence-resource catalog for `evidenceResourceId`, `evidenceResourceUri`, `evidenceResourceTitle`, `evidenceResourceDescription`, and `evidenceReadToolName`, with schema enums matching the MCP manifest, Tool Registry, Agent Packet allowed-tool metadata, and artifact index. Agents can read the same generated artifact through `visual-hive snapshot`, `visual-hive://control-plane-snapshot`, or the read-only `visual_hive_read_control_plane_snapshot` MCP tool.
- Schema/catalog health from `.visual-hive/schema-catalog.json`, including checked schema counts, failed drift checks, evidence-resource counts, read-tool counts, artifact links, raw evidence, and a guarded `schemas-verify` runbook action. This makes JSON Schema, MCP resource, Tool Registry, Agent Packet, Context Ledger, artifact-index, and Control Plane evidence-resource drift visible in the Configure and Expert areas instead of hiding it in maintenance scripts.
...
- Setup progress and recommendations from `.visual-hive/recommendations.json` and `.visual-hive/setup-pr-plan.json` plus the current config, plan, report, mutation, triage, workflow, provider, and readiness artifacts. The Setup tab shows the current phase, percent complete, blocked/review counts, next best action, evidence, commands, and artifact links before the detailed checklist for repository inspection, Playwright presence, PR-safe target selection, detected app route hints, detected Storybook story routes, existing workflow hints, starter contracts, PR safety, setup PR file plan, generated workflow previews, setup action plan, setup file generation, local validation, setup profile selection, provider posture, cost estimate, permission guidance, setup PR guidance, and guarded actions to write the recommended config and repo setup docs
- Setup-only MCP readiness from `.visual-hive/mcp-manifest.json` when a repository has setup recommendations but no committed `visual-hive.config.yaml` yet. The Setup tab shows whether first-party MCP resources for setup recommendations, setup PR plan, artifact index, and the MCP manifest itself are available, confirms `externalCallsMade: 0`, lists read-only setup tools, and keeps execution/provider tools visibly disabled. This gives agents and new users a guided, no-network evidence path before any config, workflow, provider, baseline, or issue-creation writes happen.
...
- Run history trends from `.visual-hive/history.json`, including latest deterministic status, tracked run count, mutation score trend, baseline churn, visual diff totals, trend reasons, and archived artifact links. This is longitudinal review evidence only: it helps users and agents spot flake, regression patterns, baseline instability, runtime/cost drift, and weak mutation adequacy, but it does not override the current Visual Hive verdict.
- Catalog-backed artifact links in the Review and Configure areas prefer the shared artifact index metadata (`evidenceResourceId`, `evidenceResourceUri`, `evidenceReadToolName`) when present, so users, agents, MCP resources, and schemas all refer to the same evidence object instead of separate hand-written paths.
- The Expert console also reads `.visual-hive/context-ledger.json` directly when present and shows Context Ledger `toolCalls[].evidenceResources` as linked evidence resources. This lets humans and agents follow multi-artifact steps such as triage from one command to the triage report, issue body, PR comment, prompts, and missing-test guidance without scraping raw JSON paths.
- The same catalog-backed link path is used for readiness gates, workflow audits, baseline review queues, provider results, Argos upload manifests, provider-specialist packets, repair prompts, and the Context Ledger when those artifacts exist.
...
- Triage artifacts are first-class catalog-backed evidence: `.visual-hive/triage.json`, `issue.md`, `pr-comment.md`, `triage-prompt.md`, `repair-prompt.md`, and `missing-tests.md` expose shared resource IDs and read-tool names so agents can consume them without posting to GitHub, calling an LLM, editing tests, or changing the verdict.
...
- Setup PR plan review from `.visual-hive/setup-pr-plan.json`, including planned files, validation commands, provider posture, generated workflow security checks, and `externalCallsMade: 0`
...
- Provider upload visibility, including optional Argos `.visual-hive/provider-results.json` and `.visual-hive/provider-upload/argos/manifest.json` evidence with upload status, staged/uploaded artifact counts, external calls, and sanitized provider output. The provider-specialist Agent Packet panel renders allowed tools with catalog-backed resource titles, URIs, and artifact links, so users can see exactly which local evidence a provider-review agent may read.
...
- Agent-forward handoff visibility, including Evidence Packet, Visual Hive verdict, Hive handoff packet, Hive-native export, Hive export mode comparison, test-creation plan, repair/test Agent Packet, handoff-agent Agent Packet, provider-specialist Agent Packet, Tool Registry, and Context Ledger artifacts. The Hive-native bundle panels read `.visual-hive/hive/hive-export.json` and show no-network export status, mode, external call count, bead/fact/graph/work-order counts, blocked reasons, artifact links, top Hive Beads, knowledge graph edges, wiki-vault facts, and guarded repair work-order constraints.
- Agent packet-chain artifact links prefer catalog-backed metadata for Evidence Packet, Verdict, Handoff Packet, Hive Export, guarded repair artifacts, Agent Packet, and test-creation plan resources. Non-catalog Markdown summaries are still shown as ordinary artifact paths.
- A Hive readiness next-step card derived from `.visual-hive/evidence-packet.json`, showing the safest recommended Hive mode, its status, the reason, the next command, whether local preview is allowed, whether a trusted workflow is required, and blocked policy reasons.
- Hive export mode policy from both `.visual-hive/evidence-packet.json` and `.visual-hive/hive/mode-comparison.json`, including the Evidence Packet's pre-export `recommendedMode`, recommendation reason, per-mode readiness, side-by-side no-network previews for `advisory`, `measured`, `repair_request`, `guarded_repair`, and `full` modes, emitted artifact counts, comparison artifact links, and fixed preview commands. Beginner mode highlights the first three local preview choices; expert mode exposes guarded and full blocked policy states. The UI explains what each mode emits, keeps `guarded_repair` and `full` as policy states rather than casual local actions, and never calls Hive from the browser.
...
- Safe raw artifact browser for `.visual-hive`, with image previews, redacted text previews, and shared evidence-resource metadata from the core artifact index. Known artifacts such as the Evidence Packet, Verdict report, Control Plane snapshot, Agent Packet, Tool Registry, Context Ledger, provider results, Argos upload manifest, pipeline status, and artifact index expose the same resource IDs, URIs, titles, and MCP read tool names used by the MCP manifest and Tool Registry.
...
- Provider decision recording writes only catalog-backed `.visual-hive/provider-decisions.json` (`visual-hive://provider-decisions` / `visual_hive_read_provider_decisions`), records `externalCallsMade: 0`, and does not enable credentials, billing, uploads, provider gating, verdict overrides, or provider network calls.
...
- Hive-native export display is artifact-only. The Control Plane reads `.visual-hive/hive/hive-export.json`, `beads.json`, `knowledge-facts.json`, `knowledge-graph.json`, `wiki-index.json`, `issue-context.md`, `repair-work-orders.json`, `hive-agent-policy.json`, `mode-comparison.json`, `mode-comparison.md`, per-mode preview directories, and wiki-vault pages when present, but it does not call Hive, create Beads, or execute repair work. Repair rows always show the Visual Hive-owned final validation command and forbidden actions so the UI does not imply autonomous pass/fail authority.
- Hive export mode preview actions execute only allowlisted `visual-hive hive export --dry-run --mode ...` commands for `advisory`, `measured`, and `repair_request`, plus `visual-hive hive compare-modes` for the no-network comparison artifact. `guarded_repair` and `full` are intentionally displayed as governed future/trusted states and are not launched from the local UI.
...
The default dogfood command, `npm run demo:all`, now uses the operational pipeline as the authoritative acceptance path instead of rerunning every low-level command twice. It still generates the management artifacts this UI consumes: planning, deterministic report, mutation adequacy, baseline queue, coverage, flow/target/contract/schedule audits, workflow/provider/security/cost/risk evidence, prompt artifacts, issue/PR markdown, Evidence Packet, Verdict report, Hive handoff dry run, Agent Packet, tool registry, KubeStellar planning dogfood, raw artifact index, and Control Plane smoke evidence. For a slower command-by-command sweep, use `npm run demo:acceptance:exhaustive`. Use `npm run smoke:ui:browser` after `npm run ui:build` and artifact generation when you need a real Chromium check of the rendered guided cockpit, verdict panel, Review navigation, and Expert evidence disclosure.

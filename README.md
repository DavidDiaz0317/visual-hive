...
- `visual-hive analyze`: scans a target repo and writes catalog-backed `.visual-hive/repo-map.json` (`visual-hive://repo-map` / `visual_hive_read_repo_map`) plus `.visual-hive/repo-context.md` (`visual-hive://repo-context` / `visual_hive_read_repo_context`) with package manager, workspace, script, framework, selector, route, workflow, test-tool, target-hint, risk, and coverage-gap evidence.
...
- `visual-hive mcp --describe|--stdio`: exposes first-party read-only MCP resources over existing artifacts such as config, plan, report, Evidence Packet, Control Plane snapshot, verdict, handoff, Hive export, guarded repair preview, repair request envelope, trusted repair consumer summary, trusted repair workflow dry-run, Hive mode comparison, provider results/upload manifest evidence, Agent Packet, Tool Registry, Context Ledger, pipeline status, schema catalog, repo map/context, MCP manifest, and artifact index. Use `visual-hive mcp --repo <path> --describe --output .visual-hive/mcp-manifest.json` for a setup-only manifest before a config exists; that mode exposes setup recommendations, setup PR plan, repo map, repo context, artifact index, and MCP manifest evidence only, and does not start an MCP stdio server. Execution and provider-upload tools remain disabled by default.

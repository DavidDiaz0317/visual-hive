...
- `visual-hive hive export --dry-run`: consumes `.visual-hive/evidence-packet.json` plus optional `.visual-hive/handoff.json`, then writes `.visual-hive/hive/hive-export.json`, `beads.json`, `knowledge-facts.json`, `knowledge-graph.json`, `issue-context.md`, `repair-work-orders.json`, `hive-agent-policy.json`, and `wiki/*.md` for Hive ingestion or trusted repair workflows. Advisory mode emits issue context only; measured mode emits beads/facts/graph; repair modes emit PR-only work orders that require a fresh Visual Hive pass before merge.
- `visual-hive hive compare-modes`: writes a no-network comparison artifact for advisory, measured, and repair-request Hive export modes so maintainers can inspect what each mode would send to Hive before enabling a trusted workflow.
- `visual-hive test-creation-plan`: consumes `.visual-hive/evidence-packet.json`, optional `.visual-hive/coverage-recommendations.json`, and optional `.visual-hive/handoff.json`, then writes `.visual-hive/test-creation-plan.json` and `.visual-hive/test-creation-plan.md` with advisory no-write test recommendations.
...
- `visual-hive mcp --describe|--stdio`: exposes first-party read-only MCP resources over existing artifacts such as config, plan, report, Evidence Packet, verdict, handoff, Hive export, Hive mode comparison, Agent Packet, Tool Registry, Context Ledger, pipeline status, and artifact index. Execution tools remain disabled by default.
